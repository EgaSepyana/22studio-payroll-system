...
export async function updateTask(taskId, { divisi, description, target_qty }) {
  ...
  const updated = await TasksRepo.updateById(taskId, (task) => {
    ...
    if (target_qty !== undefined) {
      ...
      // Uses the new divisi if it's changing in this same patch, since
      // Cutting's pending_audit vs completed distinction depends on it.
      patch.status = taskStatusFromQty(completedQty, nextTarget, divisi !== undefined ? divisi : task.divisi);
    }
    return patch;
  });
